...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Add the maintenance amount to the total amount
...
<body>
    ...
    <div class="main-content">
        ...
        <!-- Maintenance Form -->
        <form method="POST" action="m_admin.php">
            <div class="maintenance-info">
                ...
                <div class="form-group-horizontal">
                    <div class="form-item">
                        ...
                        <input type="number" id="maintenance-amount" name="maintenance-amount" required placeholder="Enter Maintenance Amount" />
                    </div>
                </div>
    ...
    document.querySelectorAll('input[name="payment-duration[]"]').forEach(function (checkbox) {
        checkbox.addEventListener('change', function () {
            ...
            if (this.checked) {
                ...
                // Keep the end date visible
                document.getElementById('e_date').value = ''; // Reset end date value
            }
        });
    });

    // JavaScript for updating end date
    document.getElementById('s_date').addEventListener('change', function() {
        var startDate = new Date(this.value);  // Get the start date value
        var paymentDuration = document.querySelectorAll('input[name="payment-duration[]"]:checked'); // Get checked durations
        var endDate = document.getElementById('e_date');
        
        if (paymentDuration.length === 1) {  // Ensure only one duration is selected
            paymentDuration.forEach(function(duration) {
                if (duration.value === '1-month') {
            ...
            // Update the end date input field with the calculated end date
            var month = startDate.getMonth() + 1;  // Get month as 1-12
            var day = startDate.getDate();
            var year = startDate.getFullYear();
            endDate.value = `${year}-${month < 10 ? '0' + month : month}-${day < 10 ? '0' + day : day}`;
        } else {
            // If no or more than one duration is selected, don't change the end date
            endDate.value = '';
        }
    });
    </script>
